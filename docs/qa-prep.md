...
2. 必须说清两层：**演示环境用的 user/resource/behavior 是 engine `dataset/sim` 的模拟数据**，通过 `demo_seed` 播种（用户名形如 `demo1`），**不是真实用户数据**；真实库里目前只有少量真实账号与行为。
...
2. ❌「我们有大量真实用户行为数据」 —— 真实前端只上报 `view`，`click`/`favorite` 只在 mock 与类型定义里；演示数据是 engine 的 **sim 模拟数据**，通过 `demo_seed` 播种，用户名是 `demo1`/`demo2` 这种，**绝不能当真实用户数据讲**。
...
> 「数据多少取决于跑过哪条链路，我给两组实测口径。」

| 口径 | resources | users | admins | recommendations | 说明 |
|---|---|---|---|---|---|
| 课题当前口径（以本任务给定为准） | **153 条，全部 B 站 video，id 1..153** | **2 条** | **0 行（空表）** | **仅 1 行** | 结论：**当前首页不是个性化推荐，而是评分兜底**；且 admins 为空说明此刻没有管理员账号 |
| 本机开发库实测 | 500（article 111 / course 111 / video 278） | 2001 | 1 | 2001 | 跑过 `demo_seed -with-behaviors`，掺入 engine `dataset/sim` 模拟数据，行为约 10 万条、评分约 9840 条 |
| 历史快照 `data/snapshots/20260916_181925/meta.json` | 139 | 2 | — | — | 导出时刻的真实库：behaviors 14 条、ratings 0 条、categories 2 个 |

**要点**：
1. 两组数字都不假，但**语义不同**——开发库那 500/2001 里绝大部分是 **sim 模拟数据**，绝不能当真实用户量讲。
2. 说「当前库 153 资源 / 2 用户」时，顺带说明「这是 B 站采集后的真实库口径；`recommendations` 只有 1 行，说明 engine 结果基本没导入过，首页现在走的是兜底」。
3. `admins` 为空 → 导入接口需要管理员，得先有管理员账号（`demo_seed -admin` 或手工插 `admins` 行）。

**依据**：本机 `edurec` 库实测查询；`backend/data/snapshots/20260916_181925/meta.json:17-20`；`backend/cmd/demo_seed/main.go:21-29`；`backend/internal/middleware/admin.go:20-30`（管理员判定依赖 `admins` 表）。
